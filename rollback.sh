...
# Restores a pre-install snapshot (taken automatically before each package
# install). Includes all Python files, templates, static files, and the DB.
#
...
if [ -f "$SNAP_DIR/netwatch.db" ]; then
    ...
    # Write restore script to a temp file
    ...
    printf '    print("  FAILED: database restore: " + str(e))\n' >> "$TMPPY"
    printf '    sys.exit(1)\n' >> "$TMPPY"
...
if sudo systemctl start netwatch-web.service 2>/dev/null; then
    ...
    echo "  FAILED: netwatch-web failed to start"
    FAILED=$((FAILED + 1))
fi
...
if [ $FAILED -eq 0 ]; then
    ...
    echo "  WARNING: Rollback finished with $FAILED failure(s)"
